"""
Saliency-Augmented Multi-Pattern Pooling modules.
...
class TriangularPattern(nn.Module):
    ...
    def forward(self, x, s):
        ...
        up_indices = torch.triu_indices(x.shape[2], x.shape[3], device=x.device, offset=1)
        up_feat = x[:, :, up_indices[0], up_indices[1]].mean(dim=2)
        lw_indices = torch.tril_indices(x.shape[2], x.shape[3], device=x.device, offset=-1)
        ...
        s = self.downsample(s)
        up_sal_indices = torch.triu_indices(s.shape[2], s.shape[3], device=s.device, offset=1)
        up_sal = s[:, :, up_sal_indices[0], up_sal_indices[1]].flatten(1)
        lw_sal_indices = torch.tril_indices(s.shape[2], s.shape[3], device=s.device, offset=-1)
...
class SurroundPattern(nn.Module):
    def __init__(self, crop_size=1.0 / 2):
        super().__init__()
    ...
    def forward(self, x, s):
        ...
        inside_feat = x[:, :, inside_indices[0], inside_indices[1]].mean(2)
        outside_indices = torch.where(~x_mask)
        outside_feat = x[:, :, outside_indices[0], outside_indices[1]].mean(2)
        ...
        if outside_sal.shape != inside_sal.shape:
            outside_sal = F.adaptive_max_pool1d(outside_sal.unsqueeze(1), output_size=inside_sal.shape[1]).squeeze(1)
...
class HorizontalPattern(nn.Module):
    ...
    def forward(self, x, s):
        H = x.shape[2]
        up_feat = x[:, :, :H // 2, :].mean(3).mean(2)
        lw_feat = x[:, :, H // 2:, :].mean(3).mean(2)
        fused = torch.stack([up_feat, lw_feat], dim=2).unsqueeze(3)
        if s is None:
            return fused
        SH = s.shape[2]
...
class VerticalPattern(nn.Module):
    ...
    def forward(self, x, s):
        W = x.shape[3]
        left_feat = x[:, :, :, :W // 2].mean(3).mean(2)
        right_feat = x[:, :, :, W // 2:].mean(3).mean(2)
...
class MultiDirectionPattern(nn.Module):
    def __init__(self):
        super().__init__()
        feat_mask = self._generate_mask(7, 7)
        sal_mask = self._generate_mask(56, 56)
        self.register_buffer('feat_mask', feat_mask)
        self.register_buffer('sal_mask', sal_mask)

    @staticmethod
    def _generate_mask(w, h):
...
class MPPModule(nn.Module):
    ...
    def pattern0(self):
        pool = GlobalPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (1, 1), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern1(self):
        pool = HorizontalPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (2, 1), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern2(self):
        pool = VerticalPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (1, 2), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv
    ...
    def pattern6(self):
        pool = QuarterPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (2, 2), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv
    ...
    def pattern8(self):
        ...
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (3, 3), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern9(self):
        pool = HorThirdPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (1, 3), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern10(self):
        pool = VerticalThirdPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (3, 1), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern11(self):
        pool = MultiDirectionPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim, self.out_dim, (2, 4), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv
...
class SAMPPModule(nn.Module):
    def __init__(self, in_dim, out_dim, saliency_size, dropout=0.5,
                 pattern_list=None, fusion='sum'):
        super().__init__()
        if pattern_list is None:
            pattern_list = [1, 2, 3, 4, 5, 6, 7, 8]
    ...
    def pattern0(self):
        ...
        pool = GlobalPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (1, 1), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern1(self):
        sal_length = (self.saliency_size // 2) ** 2
        pool = HorizontalPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (2, 1), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern2(self):
        sal_length = (self.saliency_size // 2) ** 2
        pool = VerticalPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (1, 2), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv
    ...
    def pattern6(self):
        sal_length = (self.saliency_size // 2) ** 2
        pool = QuarterPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (2, 2), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv
    ...
    def pattern8(self):
        ...
        pool = ThirdOfRulePattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (3, 3), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern9(self):
        sal_length = (self.saliency_size // 3) * self.saliency_size
        pool = HorThirdPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (1, 3), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern10(self):
        sal_length = (self.saliency_size // 3) * self.saliency_size
        pool = VerticalThirdPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (3, 1), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv

    def pattern11(self):
        sal_length = int((self.saliency_size // 2) * (self.saliency_size // 2 + 1) / 2)
        pool = MultiDirectionPattern()
        conv = nn.Sequential(nn.Conv2d(self.in_dim + sal_length, self.out_dim, (2, 4), bias=False), nn.ReLU(True), nn.Flatten(1))
        return pool, conv
